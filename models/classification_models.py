...
from models.LSTM import LSTMAttentive, LSTMCustom, LSTMSimple, LSTMSpatial
...
class MultimodalAttentionRNN(nn.Module):
    def __init__(self, ans_vocab, glimpse=2, dropout=0.5, joint=False):
        ...
        self.joint = joint
        self.blockv = MLBBlockVAttention(
            2400, 1200, glimpse, dropout=dropout, joint=joint
        )
        ...
        if self.joint:
            self.blockq = MLBBlockQAttention(
                2048, 1200, glimpse, dropout=dropout, joint=joint
            )
            ...
            self.question_skip = nn.Sequential(
                nn.Dropout(dropout), nn.Linear(2400, 1200 * glimpse)
            )
...
class MLBBlockVAttention(nn.Module):
    def __init__(self, input_size, output_size, glimpse, dropout, joint=False):
        ...
        self.question_attn = nn.Sequential(
            nn.Dropout(dropout), nn.Linear(input_size, output_size), nn.Tanh()
        )
        self.attn_block = nn.Conv2d(
            output_size, glimpse, kernel_size=1, stride=1, padding=0
        )
        ...
        if not joint:
            self.question_embed = nn.Sequential(
                nn.Dropout(dropout), nn.Linear(2400, output_size * glimpse), nn.Tanh()
            )
        self._initialize_weights()
...
class MLBBlockQAttention(nn.Module):
    def __init__(self, input_size, output_size, glimpse, dropout, joint=False):
        ...
        self.visual_attn = nn.Sequential(
            nn.Dropout(dropout), nn.Linear(2048, output_size), nn.Tanh()
        )
        self.question_attn = nn.Sequential(
            nn.Dropout(dropout), nn.Linear(2400, output_size), nn.Tanh()
        )
        ...
        if not joint:
            self.visual_embed = nn.Sequential(
                nn.Dropout(dropout), nn.Linear(2048, output_size * glimpse), nn.Tanh()
            )
        self._initialize_weights()

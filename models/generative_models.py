...
class G_Spatial_Adversarial(nn.Module):
    ...
    def forward(
        ...
        if teacher_forced:
            return self._forward_forced_cell(
                image_features, text_features, answer, answer_lengths, states, skip=2
            )
        else:
            return self._forward_free_cell(
                image_features, text_features, answer_lengths, states, skip=2
            )

    def _forward_forced_cell(
        self, image_features, text_features, answer, answer_lengths, states, skip=2
    ):
        ...
        hiddens_tensor = Variable(
            torch.cuda.FloatTensor(len(answer_lengths), max_length, self.hidden_size)
        )
        context_tensor = Variable(
            torch.cuda.FloatTensor(len(answer_lengths), max_length, self.hidden_size)
        )
        hx, cx = states
        hx, cx = hx[0], cx[0]
        ...
        results_packed, _ = pack_padded_sequence(
            results_tensor, answer_lengths, batch_first=True
        )
    ...
    def _forward_free_cell(
        self, image_features, text_features, answer_lengths, states, skip=2
    ):

        max_length = max(answer_lengths)
        output_tensor = Variable(
            torch.cuda.FloatTensor(len(answer_lengths), max_length, self.vocab_size)
        )
        ...
        output_tensor, _ = pack_padded_sequence(
            output_tensor, answer_lengths, batch_first=True
        )
    ...
    def beamSearch(self, features, states, n=1, diverse_gamma=0.0):
        ...
        hx, cx = states
        hx, cx = hx[0], cx[0]
        hx, cx = self.lstm_attention(
            inputs, hx, cx, features_local
        ...
        for word_index in range(50):
            ...
            for choice_index, choice in enumerate(best_choices):
                ...
                out_states, out_context = self.lstm_attention(
                    inputs,
                    cached_states[choice_index],
                    cached_context[choice_index],
                    features_local,
                )  # (batch_size, 1, hidden_size)
                ...
                inner_confidences, inner_best_choices = (
                    inner_confidences.squeeze(),
                    inner_best_choices.squeeze(),
                )
            ...
            best_choices_index = [
                ...
                if end_idx not in best_list[index].data.int()
            ]
            ...
            terminated_confidences.extend(
                [
                    best_confidence_tensor[index].data.cpu().numpy()[0]
                    for index in terminated_index
                ]
            )
            ...
            if len(best_choices_index) > 0:
                ...
                confidences = [
                    best_confidence_tensor[index] for index in best_choices_index
                ]
        ...
        # print(best_choices)
        if len(best_choices_index) > 0:
            terminated_choices.extend(
                [best_list[index] for index in best_choices_index]
            )
            terminated_confidences.extend(
                [
                    best_confidence_tensor[index].data.cpu().numpy()[0]
                    for index in best_choices_index
                ]
            )

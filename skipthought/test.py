...
class Tester:
    ...
    def launch_tests(model):
        methods = [
            ...
            if callable(getattr(Tester, func)) and "test_" == func[:5]
        ]
        for name in methods:
            print(name)
    ...
    def _test_skipthoughts(model, output, fname):
        ...
        if is_uniskip(model):
            ...
            print(
                "Unkown model of type: {} {}".format(
                    model.__class__, model.__classes__.__bases__
                )
            )
    ...
    def test_oneWord(model):
        ...
        input.data[0, 0] = 1
        output = model(input)
        Tester._test_skipthoughts(
            model, output, "features_oneWord_normFalse_eosFalse.npy"
        )
    ...
    def test_oneWord_eos(model):
        ...
        Tester._test_skipthoughts(
            model, output, "features_oneWord_normFalse_eosTrue.npy"
        )

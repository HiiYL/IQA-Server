...
from skipthought.gru import GRU, BayesianGRU
...
class UniSkip(AbstractUniSkip):
    ...
    def _load_rnn(self):
        self.rnn = nn.GRU(
            input_size=620, hidden_size=2400, batch_first=True, dropout=self.dropout
        )
...
if __name__ == "__main__":
    ...
    input = Variable(
        torch.LongTensor(
            [[6, 1, 2, 3, 3, 4, 0], [6, 1, 2, 3, 3, 4, 5], [1, 2, 3, 4, 0, 0, 0]]
        )
    )
    print(input.size())
    model.eval()
